...
Citizen.CreateThread(function()
	...
	promptGroupTitleNoKey = CreateVarString(10, "LITERAL_STRING", "Door (No Key)")
	promptGroupTitleMasterKey = CreateVarString(10, "LITERAL_STRING", "Door (Master Key)")
...
function handlePrompts(doorState,hasKey)
	if secDoor ~= nil then
	...
	if doorState == 1 or doorState == -1 then
		...
		if hasKey == true or hasKey == "master" then
...
function handleNearbyDoor()
	...
	while inRange == true and switch == false and doorMode == true do
		...
		handlePrompts(doorState,hasKey)
		if hasKey == true then
			PromptSetActiveGroupThisFrame(promptGroup, promptGroupTitle)
		elseif hasKey == "master" then
			PromptSetActiveGroupThisFrame(promptGroup, promptGroupTitleMasterKey)
		else
			PromptSetActiveGroupThisFrame(promptGroup, promptGroupTitleNoKey)
		end
